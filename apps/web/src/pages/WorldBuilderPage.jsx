...
const WorldBuilderPage = () => {
  ...
  return (
    <AppLayout>
      ...
      <ProtectedFeatureRoute feature="world_builder" featureName="World Builder">
        <PremiumFeatureGate feature="world_builder" featureName="World Builder">
          <div className="h-[calc(100vh-6rem)] flex flex-col -m-8"> {/* Negative margin to fill AppLayout padding */}
            
            {error ? (
              <div className="flex-1 flex flex-col items-center justify-center bg-card/30 m-8 rounded-xl border border-destructive/30">
                <AlertCircle className="h-12 w-12 text-destructive mb-4" />
                <p className="text-lg mb-4">{error}</p>
                <Button onClick={fetchData} variant="outline">Retry</Button>
              </div>
            ) : isLoading ? (
              <div className="flex-1 flex m-8 gap-4">
              ...
              <div className="flex-1 flex w-full h-full overflow-hidden">
                {/* Left Sidebar */}
                <div className="w-72 shrink-0 h-full z-10 shadow-xl">
                  <WorldBuilderSidebar 
                    locations={locations}
                    regions={regions}
                ...
                {/* Right Panel */}
                <div className={`w-80 shrink-0 h-full z-10 shadow-xl transition-transform duration-300 ${selectedLocation ? 'translate-x-0' : 'translate-x-full absolute right-0'}`}>
                  <LocationDetails 
